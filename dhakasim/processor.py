...
class Processor:
    ...
    def _generate_an_object(self, link, object_type: int) -> None:
        ...
        # A blockage deeper than the whole carriageway cannot stand on it, and
        # probing for a gap would run the strip index off the end -- negative
        # in the reverse branch, past the last strip in the forward one.  The
        # Java original throws IndexOutOfBoundsException here; it never fired
        # because every surveyed link is far wider than any sampled blockage,
        # but an OSM residential street of a few metres is not.
        if number_of_occupied_strips + 1 > random_segment.number_of_strips():
            return
    ...
    def _read_network(self) -> None:
        ...
        names = {}
        try:
            with open(self.input_path("node_names.txt"), "r",
        ...
        link_names = {}
        try:
            with open(self.input_path("link_names.txt"), "r",
                      encoding="utf-8") as reader:
                for line in reader:
                    parts = line.split(None, 1)
                    if len(parts) == 2:
        ...
        if not Parameters.PLACE_NAME:
            place = ""
            try:
                with open(self.input_path("place.txt"), "r",
                          encoding="utf-8") as reader:
                    for line in reader:
                        line = line.strip()
