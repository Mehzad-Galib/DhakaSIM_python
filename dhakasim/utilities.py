...
from .javacompat import (Color, GammaDistribution, JavaRandom, jbool, jdiv,
                         jexp, jint, jmin, jround, lines_intersect)
...
# The return_x3..y6 family divides by the segment length without guarding
# zero.  Java doubles do not raise there: 0/0 is NaN, which Math.round then
# turns into 0, so the original draws a degenerate corner and carries on.
# jdiv reproduces that -- a zero-length vehicle segment (seen on miami) must
# not crash the paint, and callers who need better than the Java behaviour
# test segment length themselves, as road_geometry does.
...
def initialize() -> None:
    ...
    try:
        ...
        Parameters.BASE_ACROSS_PEDESTRIAN_MODE = Parameters.across_pedestrian_mode
