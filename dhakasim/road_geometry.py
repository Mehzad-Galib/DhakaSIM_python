...
def carriageways(link_list, pixel_per_meter):
    ...
    for link in link_list:
        ...
        for j in range(link.get_number_of_segments()):
            ...
            # perpendicular, so a zero-length segment is dropped rather than
            # asked for kerb corners.
...
def build(link_list, node_list, pixel_per_meter, widen=0.0):
    ...
    *widen* adds that many metres to every carriageway, for drawing only --
    ...
    return (quads,
            ...
            carriageways(link_list, pixel_per_meter))
