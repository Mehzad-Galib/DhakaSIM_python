...
class Parameters:
    ...
    #: The same capture for the other settings a network's defaults.txt may
    #: pin (the arterial networks switch pedestrians and roadside objects
    #: off), so the intersection dropdown can put them back when it moves to
    #: a network that states none of its own.
    BASE_ACROSS_PEDESTRIAN_MODE = False
    BASE_ALONG_PEDESTRIAN_MODE = False
    BASE_OBJECT_MODE = False
    BASE_SIGNAL_CHANGE_DURATION = 1
